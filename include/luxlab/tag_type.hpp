#ifndef LUXLAB_TAG_TYPE_HPP
#define LUXLAB_TAG_TYPE_HPP
...
class TagType {
   public:
    ...
    TagType() = default;
    constexpr TagType(Type type) : m_type(type) {}
    constexpr explicit TagType(int type) : m_type(static_cast<Type>(type)) {}
...
template <>
struct formatter<luxlab::TagType> : formatter<std::string> {
    format_context::iterator format(const luxlab::TagType& type,
                                    format_context& ctx) const;
};

}  // namespace fmt

#endif  // ! LUXLAB_TAG_TYPE_HPP
